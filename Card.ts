class Card {
    ...
    private _wrapper: HTMLElement | Document;
    private _range: { from: number, to: number } = { from: 0, to: 0 };
    private _dragging: boolean = false;
    private _moveData: { x: number, y: number }[] = [];

    constructor(className: string, wrapperId?: string) {
    ...
    private setRange() {
        if (this._wrapper === document) {
            ...
            const rect = (this._wrapper as HTMLElement).getBoundingClientRect();
            this._range = {
                from: rect.left + ((this._wrapper as HTMLElement).offsetWidth / 3),
                to: rect.right - ((this._wrapper as HTMLElement).offsetWidth / 3)
            }
        }
    }

    private applyMouseEvent(that: Card) {
        ...
        this._wrapper.addEventListener('mousemove', function(e) {
            e.preventDefault();
            if (that._dragging) {
                const target: HTMLElement = document.querySelector(`.${that._className}-dragging`);
    ...
    swipeCard(e: Event) {
        const card: HTMLElement = document.querySelector(`.${this._className}-dragging`);
